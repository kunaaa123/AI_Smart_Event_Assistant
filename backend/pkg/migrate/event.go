...
// MigrateEventImages moves files to uploads/events and updates DB.
// dsn and uploadsDir are passed so script is configurable.
func MigrateEventImages(dsn, uploadsDir string) error {
	...
	eventsDir := filepath.Join(uploadsDir, "events")
	if err := os.MkdirAll(eventsDir, 0755); err != nil {
		return fmt.Errorf("mkdir events: %w", err)
	}

	rows, err := db.Query("SELECT image_id, image_url FROM event_images")
	if err != nil {
	...
	for _, r := range recs {
		...
		newPath := filepath.Join(eventsDir, newName)
		...
		newURL := "/uploads/events/" + newName
		if _, err := db.Exec("UPDATE event_images SET image_url = ? WHERE image_id = ?", newURL, r.id); err != nil {
